...
function EquipmentCard({ eq, to }: { eq: Equipment; to: string }) {
...
export default function EquipmentPage() {
  ...
  // Show all equipment for now - filter can be enabled later
  const activeEquipment = useMemo(() => {
    return allEquipment;
    // To enable active filter: return allEquipment.filter((e) => isActiveEquipment(e.name));
  }, [allEquipment]);
    ...
    <div className="space-y-6">
      ...
      {activeEquipment.length === 0 && allImplements.length === 0 ? (
        <EmptyState
          icon={Tractor}
          title="No Equipment"
          description="Sync from John Deere to see your equipment fleet."
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {activeTab === 'implements'
            ? filteredImplements.map((impl) => <ImplementCard key={impl.id} impl={impl} />)
            : activeTab === 'all'
              ? [
